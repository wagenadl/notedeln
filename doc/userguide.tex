...
install'' to install into ``/usr/local/bin''. On Mac, ``make macdmg''
...
pressing ``Control''+``V'' to paste an image from the system
clipboard. 
...
your annotation by selecting the ``hand'' icon (``F3'' or ``Control) and
...
``Control''+``Backslash'' (Figure~\ref{fig:texcodes}).
%
\begin{figure}
...
\caption{{\bf Symbols that may be obtained by a TeX-like
  ...
  sequence on the right and press ``Control''+``Backslash.''
...
typing {\tt +}, {\tt ++}, {\tt @}, {\tt\$}, and
{\tt \#} before pressing ``Control''+``N.'' Footnotes are deleted by deleting the tag in the main
text or by pressing ``Control''+``Shift''+``N'' while the tag is
highlighted.
...
Press ``Control''+``L'' to create a hyperlink. (If your hyperlink
...
single-character words are typeset in italics.\footnote{A few exceptions
  apply: ``x'' does not generate $\times$, ``d'' does not generate
  $\partial,$ and Eln tries to guess whether you mean the variables
  $a$ and $I$ or the words ``a'' and ``I.'' A few more subtle cases
  are handled semi-intelligently as well.} Additionally, simple
